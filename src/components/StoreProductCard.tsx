...
/**
 * Product card for non-UberEats stores (Sr. del Queso, Carne Vecino).
 * Similar to ProductCard but:
 * - No brand display
 * - No default quantity shown when unselected
 * - Starts at qty 1 when selected
 */
export default function StoreProductCard({
  ...
  const handleTap = useCallback(() => {
    if (!product.selected) {
      onToggle(product.id, true, 1);
    } else {
      onToggle(product.id, false, 1);
    }
  }, [product, onToggle]);
